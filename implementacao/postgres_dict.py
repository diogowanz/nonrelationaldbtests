	...
	def insereDocEmpregado(self,matricula,tp_documento, no_doc,file):
		...
		if resultEmpregado.arraysize != 0:
			...
			if resultado.arraysize != 0:
				...
				sql = "insert into tb006_documento (no_documento,id_tipo_documento,id_empregado,dh_upload) values ('"+no_doc+"',"+str(tp_documento)+","+str(resultEmpregado['id_empregado'])+",'"+str(datetime.datetime.now())+"')"
				cursor = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
				cursor.execute(sql)
				conn.commit()
	...
	def listaOrgaos(self,nome='',endereco='',cidade='',uf=''):
		...
		if nome == '' and endereco == '' and cidade == '' and uf == '':
			...
			if nome != '' and nome != None:
				filtro = "and no_orgao = '"+nome+"'"
			if endereco != '' and endereco != None:
				filtro = "and no_endereco = '"+endereco+"'"
			if cidade != '' and cidade != None:
				filtro = "and no_cidade = '"+cidade+"'"
			if uf != '' and uf != None:
				filtro = "and no_uf = '"+uf+"'"
